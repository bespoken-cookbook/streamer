...
var AudioManager = {
    ...
    introductorySSML: function (ready) {
        if (this.feed().hasIntroduction()) {
            ...
            encoder.encodeURLAndPublishAs(this.feed().introductionAudioURL, introductionKey, function(error, convertedURL) {
    ...
    aboutSSML: function (ready) {
        if (this.feed().aboutAudioURL) {
            ...
            encoder.encodeURLAndPublishAs(this.feed().aboutAudioURL, aboutKey, function(error, convertedURL) {
                var ssml = '<audio src="' + convertedURL + '" />' + newSessionBlurb;
                ready(ssml, newSessionBlurb);
            });
        } else {
