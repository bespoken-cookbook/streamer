...
var AudioConverter = {
    ...
    convert: function(inputData, fileType, callback) {
        ...
        tmp.file({dir: '/tmp', prefix: 'ffmpeg-', postfix: '.' + fileType }, function(error, inputPath, fd, cleanupCallback) {
            // Write the file
            fs.writeFile(inputPath, inputData, null, function () {

                // Get a temporary filename for the output file
                tmp.tmpName({ dir: '/tmp', postfix: '.mp3'}, function (error, outputPath) {

                    // Run ffmpeg to convert the file to MP3
                    child_process.execFile('ffmpeg',
                        ['-i', inputPath, '-codec:a', 'libmp3lame', '-b:a', '48k', '-ar', '16000', '-af', 'volume=2', outputPath],
