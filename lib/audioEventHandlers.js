...
var audioEventHandlers = Alexa.CreateStateHandler(constants.states.PLAY_MODE, {
    ...
    'PlaybackNearlyFinished' : function () {
        ...
        var podcast = AudioManager.feed().items[this.attributes['playOrder'][enqueueIndex]];
        ...
        if (scan) {
            if (enqueueIndex % 2 == 1) {
                ...
                self.response.audioPlayerPlay(playBehavior, silenceURL, enqueueToken, expectedPreviousToken, offsetInMilliseconds);
                self.emit(':responseReady');
            } else {
                podcast.scanAudioURL(function (error, url) {
                    if (url !== null) {
                        self.response.audioPlayerPlay(playBehavior, url, enqueueToken, expectedPreviousToken, offsetInMilliseconds);
                    }
                    self.emit(':responseReady');
                });
            }
            ...
            this.response.audioPlayerPlay(playBehavior, podcast.audioURL, enqueueToken, expectedPreviousToken, offsetInMilliseconds);
